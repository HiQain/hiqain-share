...
function parseFrameBody(body: unknown): { imageDataUrl: string; width: number; height: number } {
...
router.get("/screen-share/rooms/:code/frame", (req, res) => {
  ...
  res.json({
    ...
    frame: success.frame
      ? {
          imageDataUrl: success.frame.imageDataUrl,
...
router.post("/screen-share/rooms/:code/frame", (req, res) => {
  const body = parseFrameBody(req.body);
  const { deviceId, networkId } = getScreenDevice(req, res);
